...
int ksu_lsm_hook(struct ksu_lsm_hook *hook)
{
    ...
    lh_head = (struct list_head *)heads_addr;
    struct list_head *lh_head_end = (struct list_head *)(heads_addr + heads_size);
    pr_info("heads_addr 0x%lx head_offset 0x%lx heads_size %ld hook_offset 0x%lx\n", (unsigned long)heads_addr,
            hook->head_offset, heads_size, hook->hook_offset);

    for (; lh_head < lh_head_end; lh_head++) {
        struct security_hook_list *entry;
        list_for_each_entry(entry, lh_head, list) {
            ...
            if (current_origin == target) {
                pr_info("found %s (target %s) at head offset %ld (provided %ld)\n", hook->head_name, hook->target_name,
                        (unsigned long)lh_head - heads_addr, hook->head_offset);
                selected_entry = entry;
                selected_slot = slot;
                selected_origin = current_origin;
                break;
        ...
        if (selected_entry) {
            if (hook->offset) {
                lh_head += hook->offset;
                if (lh_head < (struct list_head *)heads_addr || lh_head >= lh_head_end) {
