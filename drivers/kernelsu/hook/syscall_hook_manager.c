...
void __init ksu_syscall_hook_manager_init(void)
{
    ...
    // Mark existing processes so the tracepoint fires for them.
    // Without this, only the kretprobe on syscall_regfunc (which may not
    // exist on all kernels) would trigger marking.
    ksu_mark_running_process_locked();
    if (ret) {
